...
def mayne(topdir, first_level_dir, first_file):
    ...
    report = javap_everything(class_files)
    print_report(report)
...
def TODELETE_parse_class_file_names(first_level_dir, class_files):
    retval = list()
...
def TODELETE_build_reverse_index(class_file_tuples):
    retval = dict()
...
def TODELETE_augment_and_separate_duplicates(reverse_index):
    identical = dict()
    conflicting = dict()
...
def javap_everything(class_files):
    retval = list()
    for class_file in class_files:
        input = class_file
        output = class_file + ".javap"
        command = "javap -c -verbose '%s' > '%s'" % (input, output)
        subprocess.check_output(command, shell=True)
        retval.append(output)
    return retval

def TODELETE_print_duplicates(reverse_index, label):
